...
- Shell hook leak detection. The Cursor plugin invokes `wispkey guard shell` through `PATH`; the bounded native guard combines known plaintext-secret and protected-environment-variable checks, returns the existing hook decision JSON, and does not require vault access or a password.
...
- Plaintext `.env` persistence after successful attachment. `wispkey env attach` imports only explicitly selected keys, requires a meaningful host allowlist for new credentials, and atomically replaces those values in the existing file with owner-only `wk_*` tokens while preserving unselected settings.
...
- The [local browser handoff preview](browser-handoff.md) uses a separate human-controlled browser profile, exact HTTPS origin checks, short-lived one-use requests, and per-fill Windows Hello verification. MCP and owner IPC never return the login payload; the dedicated native host sends it through the browser's private native-messaging pipe after approval. No other OS approval backend is implemented yet, so other platforms refuse fills. The destination page necessarily sees the password. A separate profile is not isolation from code running as the same OS owner, and the existing owner plaintext-egress tools still apply. Generated logins are saved before filling so a failed signup cannot strand an unknown password. Review dates never auto-delete credentials.
...
- `wispkey doctor` verifies owner-only file metadata on Unix. On platforms without metadata inspection support, including the current Windows path, the vault-permissions check is skipped rather than claiming verification; file writes still use the platform protection path available to WispKey.
...
- The shell guard is a best-effort pattern detector, not a shell parser or general command sandbox. It cannot detect every secret, shell transformation, indirect disclosure, or process behavior. Invalid, empty, oversized, or unrecognized hook input fails closed, and `wispkey` must be available on the plugin process `PATH`.
- Wisp tokens are capabilities, not harmless identifiers. Without a credential host allowlist or a restrictive policy, a prompt-injected agent can direct a substitution-capable request to an unintended host and cause WispKey to send the plaintext credential there. `env attach` cannot infer host allowlists, so new credentials require `--hosts` with a meaningful hostname or bounded glob. Existing matching credentials must already have a meaningful stored allowlist; empty or wildcard-only scopes such as `*` are rejected, and attachment never broadens existing restrictions.
...
- `.env` replacement and vault insertion are not one cross-resource transaction. Host and credential conflicts are preflighted before project, partition, credential, or file changes and do not rewrite the file, but a final filesystem failure can leave newly imported credentials in the vault while the original `.env` remains unchanged. Fix the filesystem error and re-run attachment; matching stored values are idempotently reused.
...
Secrets in IPC requests are resolved in-process and never returned in responses, written to tracing logs, put on argv, or shown in notifications. Known secret JSON fields are redacted before log formatting. List and add responses include names, types, tags, hosts, and project/partition metadata only.
...
Session files and remembered protectors are excluded and will not unlock a restored vault on another machine. Instance bearer secrets and bootstrap tokens cannot be restored; workers must receive newly minted secrets. A lost master password or lost backup passphrase is unrecoverable. See [`docs/vault-backup.md`](vault-backup.md).
