...
| Website Login | `wispkey login generate` | Encrypted username+password for a bound HTTPS origin. Not injected by the HTTP proxy; local browser fill preview requires human approval and Windows Hello. |
...
The [local browser handoff preview](docs/browser-handoff.md) adds Chrome/Edge and Firefox extensions for a separate human-controlled profile on Windows. Agents request a one-time fill and poll metadata-only status; the person approves with Windows Hello. The optional tray also offers **Generate website login** with a job application preset. Build/install the preview from source; it is not yet a browser-store release.
